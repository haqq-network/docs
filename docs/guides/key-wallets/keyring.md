...
```bash
haqqd keys add mykey --keyring-backend test

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(haqqd keys show mykey -a --keyring-backend test)
```
...
yes $KEYPASSWD | haqqd keys add me
yes $KEYPASSWD | haqqd keys show me
# start haqqd with keyring-backend flag
haqqd --keyring-backend=file start
```
...
[KWallet Handbook](https://docs.kde.org/stable5/en/kwalletmanager/kwallet5/kwallet5.pdf) for more
information.
