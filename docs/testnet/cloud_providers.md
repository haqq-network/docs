...
Clone and build Haqq in the droplet using `git`:

```bash
go install https://github.com/haqq-network/haqq.git
```
...
To connect the node to the existing testnet, fetch the testnet's `genesis.json` file and copy it into the new droplets config directory (i.e `$HOME/.haqqd/config/genesis.json`).
...
```bash
scp -3 root@<TESTNET_IP_ADDRESS>:$HOME/.haqqd/config/genesis.json root@<NODE_IP_ADDRESS>:$HOME/.haqqd/config/genesis.json
```

### Start the Node

Once the genesis file is copied over run `haqqd start` inside the node droplet.
