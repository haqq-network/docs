...
Learn how Haqq handles pending state queries. {synopsis}
...
Haqq is designed quite differently on this front as there is no concept of a "pending state".
Haqq uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Haqq cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/v0.34/tendermint-core/mempool.html).

## Pending State Queries

Haqq will make queries which will account for any unconfirmed transactions present in a node's
...
Learn how to deploy a Solidity smart contract on Haqq using [Truffle](./../guides/tools/truffle.md) {hide}
