...
```bash
haqqd query bank balances $MY_VALIDATOR_ADDRESS --chain-id=haqq_11235-1
```
...
```bash
haqqd keys add recipient --keyring-backend=file

# Put the generated address in a variable for later use.
RECIPIENT=$(haqqd keys show recipient -a --keyring-backend=file)
```

The command above creates a local key-pair that is not yet registered on the chain. An account is created the first time it receives tokens from another account. Now, run the following command to send tokens to the `recipient` account:

```bash
haqqd tx bank send $MY_VALIDATOR_ADDRESS $RECIPIENT 1000000aISLM --chain-id=haqq_11235-1 --keyring-backend=file

# Check that the recipient account did receive the tokens.
haqqd query bank balances $RECIPIENT --chain-id=haqq_11235-1
```

Finally, delegate some of the stake tokens sent to the `recipient` account to the validator:

```bash
haqqd tx staking delegate $(haqqd keys show my_validator --bech val -a --keyring-backend=file) 500aISLM --from=recipient --chain-id=haqq_11235-1 --keyring-backend=file

# Query the total delegations to `validator`.
haqqd query staking delegations-to $(haqqd keys show my_validator --bech val -a --keyring-backend=file) --chain-id=haqq_11235-1
```
