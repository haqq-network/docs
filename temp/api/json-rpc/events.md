...
It is possible to subscribe to `Events` via Tendermint's [Websocket](https://docs.tendermint.com/v0.33/app-dev/subscribing-to-events-via-websocket.html).
...
Haqq also supports the Ethereum [JSON-RPC](https://eth.wiki/json-rpc/API) filters calls to
...
Since Haqq runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
...
PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), Haqq needs to cast the Tendermint
