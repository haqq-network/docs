...
Learn how to deploy a simple Solidity-based smart contract to Haqq using the Truffle environment {synopsis}
...
You should see incoming deployment logs in the Haqq daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Haqq node using the `test` command:
